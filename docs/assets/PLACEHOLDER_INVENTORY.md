# Placeholder asset inventory (Phase 2–5)
...
| Testimonials | `config/testimonials.ts` | Empty catalogue | Consent-approved quotes |
| Warranty terms | Workmanship page disclaimers | Scope-based placeholder wording | Owner/legal-approved warranty terms |
| Service-area entries | `config/service-area.ts` | South Florida + Fort Lauderdale only | Additional owner-approved locations |
| Marina entries | `config/marinas.ts` | Empty unpublished directory | Approved marina references (no implied partnership) |
| Map treatment | Service Area map placeholder | Static labeled placeholder — no SDK | Approved static/map treatment |
| Resource review status | `content/resources.ts` | Educational articles with org author | Named reviewers when approved |

Config references:

- `config/home-placeholders.ts`
- `config/marketing-placeholders.ts`
- `config/projects.ts` (`published` + owner-approved only)
- `config/about.ts`
